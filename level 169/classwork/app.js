...
const app = http.createServer((req, res) => {
    ...
    const query = fullUrl.search.slice(1).split("&");
    ...
    if (fullUrl.pathname === "/cars" && method === "GET") {
        const filter = [];
    ...
    } else if (fullUrl.pathname === "/cars" && method === "POST") {
    ...
    } else if (fullUrl.pathname === "/cars" && method === "DELETE") {
        let id = parseInt(fullUrl.searchParams.get("id"));
        const cars = JSON.parse(fs.readFileSync("cars.json", "utf-8"));
        cars.splice(id, 1);
        res.writeHead(200, {
            "Content-Type": "application/json"
